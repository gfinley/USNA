    ...
    try:
        while not done2:
            ...
            for x in range(0,1):
                ...
                #print("bird created:")
                birdsStates[x][0] = dSpaceArray[0][0][0]
                birdsStates[x][1] = dSpaceArray[0][0][0]
            ...
            while  not done:
                ...
                for index in range(0,len(birds)): 
                    bird = birds[index]
                    pipe_collision = any(p.collides_with(birds[index]) for p in pipes)
                    if pipe_collision or 0 >= bird.y or bird.y >= WIN_HEIGHT - Bird.HEIGHT:
                        ...
                        birdsStates[index][0] = currState
                        climbing= 0
                        if tP(currP) > 0:
                            climbing = 1
                        catchDone( oldState, climbing , currState)
                        birdsDone[index] = True
                        #print("bird finished")
                        pipe_collision = 0
                done = True
                if x in range(0,len(birdsDone)):
                ...
                for index in range(0,len(birds)):
                    ...
                    if oldState.x != currState.x or oldState.y != currState.y or oldState.p != currState.p:
                        ...
                        bird = birds[x]
                        if nextMove == 1 and bird.msec_to_climb <  20:
                            bird.msec_to_climb = Bird.CLIMB_DURATION
                            nextMove = 0
                        birdsStates[index][1] = birdsStates[index][0]
