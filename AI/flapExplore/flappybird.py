...
pp = True
randomV = input("please enter Search Value ")
vmapName = "vMapS" + str(randomV)
rmapName = "rewardMapS" + str(randomV)
save = "saveS" + str(randomV)
#global fail
#global success 
#oldPercent = 0
#counter = 0
...
def maxExploreFunction(state):
    if (state.x,state.y,state.p) in visitedMap:
        ...
        temp = 1
    val1 = exploreFunction(getQ(state,0), temp)
    val2 = exploreFunction(getQ(state,1), temp)
    #print(" Val1: " + str(val1))
    #print(" Val2: " + str(val2))
    if val1 > val2:
        return val1
    ...
    while not done2:
        ...
        while  not done:
            ...
            if oldState.x != currState.x or oldState.y != currState.y or oldState.p != currState.p:
                ...
                if nextMove == 1 and bird.msec_to_climb < 1 :
                    bird.msec_to_climb = Bird.CLIMB_DURATION
                    nextMove = 0
                stateX = tX(currX)
                stateY = tY(currY)
